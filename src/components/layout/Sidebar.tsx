    ...
    return (
        <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
            ...
            <nav className="sidebar-nav">
                {/* Management Section */}
                <div style={{ marginBottom: isSidebarCollapsed ? '0' : '15px' }}>
                    {!isSidebarCollapsed && <div className="nav-label">Management</div>}
                    {isSidebarCollapsed && <div className="sidebar-mini-divider" />}
                    ...
                    <button className={`nav-item ${activeTab === 'webinars' ? 'active' : ''}`} onClick={() => { setActiveTab('webinars'); setMobileMenuOpen(false); }}>
                        <span className="icon">
                    ...
                    <button className={`nav-item ${activeTab === 'admin-notifications' ? 'active' : ''}`} onClick={() => { setActiveTab('admin-notifications'); setMobileMenuOpen(false); }}>
                        <span className="icon">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M22 12h-4l-3 9L9 3l-3 9H2"></path>
                        </svg>
                        </span> <span>System Logs</span>
                    </button>
                </div>

                {/* Platform Section */}
                <div style={{ marginBottom: isSidebarCollapsed ? '0' : '15px' }}>
                    {!isSidebarCollapsed && <div className="nav-label">Platform</div>}
                    {isSidebarCollapsed && <div className="sidebar-mini-divider" />}
                    ...
                    <button className={`nav-item ${activeTab === 'notifications' ? 'active' : ''}`} onClick={() => { setActiveTab('notifications'); setMobileMenuOpen(false); }}>
                        <span className="icon">
                    ...
                    <button className={`nav-item ${activeTab === 'signup-questions' ? 'active' : ''}`} onClick={() => { setActiveTab('signup-questions'); setMobileMenuOpen(false); }}>
                        ...
                        </span> <span>Signup Probes</span>
                    </button>
                </div>

                {/* Configuration Section */}
                <div style={{ marginTop: 'auto', paddingTop: '20px' }}>
                    {!isSidebarCollapsed && <div className="nav-label">Configuration</div>}
                    {isSidebarCollapsed && <div className="sidebar-mini-divider" />}
